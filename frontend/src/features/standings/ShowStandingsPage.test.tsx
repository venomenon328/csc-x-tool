...
describe('show standings evaluation', () => {
  ...
  it('renders all entries, including zero points and a shared derived interim rank', async () => {
    ...
    expect(screen.queryByText(/Punkte aus erfassten Stimmzetteln werden bei jedem Abruf/)).not.toBeInTheDocument()
    expect(screen.queryByText(/Nur abgegebene Stimmzettel fließen ein/)).not.toBeInTheDocument()
    expect(screen.queryByText(/Punktgleiche Beiträge erhalten denselben Zwischenrang/)).not.toBeInTheDocument()
