...
describe('ResultPage', () => {
  ...
  it('requires an explicit own contest participation', async () => {
    ...
    render(<App />)
    expect(await screen.findByText(/Wähle in der Contestverwaltung zuerst ausdrücklich/)).toBeVisible()
    expect(screen.getByRole('link', { name: 'CSC-Ausgabe öffnen' })).toHaveAttribute('href', '/contests')
  })
})
