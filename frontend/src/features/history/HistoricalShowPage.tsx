...
type EditablePreviewLine = HistoricalImportPreviewLine & { included: boolean }
...
export function HistoricalShowPage() {
  ...
  useEffect(() => { if (Number.isFinite(showId)) void load() }, [load, showId])
  ...
  async function previewPaste(html: string, text: string) {
    try {
      ...
      setPreview(lines.map((line) => ({ ...line, included: line.status !== 'INCOMPLETE' })))
    } catch (caught) { setError(asEntryError(caught)) }
  }
  async function importSelected() {
    ...
    setImporting(true)
    try {
      await importHistoricalEntries(showId, chosen.map((line) => ({ artist: line.artist ?? '', title: line.title ?? '', youtubeUrl: emptyToNull(line.youtubeUrl), comment: null, participantId: line.participantId ?? -1 })))
  ...
  return <Stack spacing={3}>
    ...
    <EntryEditor entry={editor} onClose={() => setEditor(undefined)} onSave={saveEntry} participants={participants} />
...
function EntryEditor({ entry, participants, onClose, onSave }: { entry: ContestEntry | null | undefined, participants: Participant[], onClose: () => void, onSave: (draft: EntryDraft) => void }) {
  ...
  useEffect(() => { if (entry !== undefined) setDraft(entry === null ? emptyDraft : { artist: entry.artist, title: entry.title, youtubeUrl: entry.youtubeUrl ?? '', comment: entry.comment, participantId: entry.participantId }) }, [entry])
...
function validPreviewLine(line: EditablePreviewLine) { return !!line.artist?.trim() && !!line.title?.trim() && line.participantId !== null }
