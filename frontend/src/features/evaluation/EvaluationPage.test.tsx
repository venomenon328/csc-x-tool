...
describe('evaluation navigation', () => {
  ...
  beforeEach(() => {
    ...
    fetchMock.mockImplementation(async (input) => {
      switch (String(input)) {
        ...
        case '/api/shows/1/published-ballots/standings': return jsonResponse({ mottoShowId: 1, votedCount: 1, notVotedCount: 1, unrecordedCount: 3, entries: [] })
  ...
  it('keeps all views under the canonical URL-addressable evaluation area', async () => {
    const user = userEvent.setup()
    window.history.pushState({}, '', '/shows/1/evaluation?view=published-ballots')
    render(<App />)

    await screen.findByRole('tab', { name: 'Veröffentlichte Stimmzettel', selected: true })
    ...
    expect(screen.getByRole('tab', { name: 'Meine Einreichung', selected: true })).toBeInTheDocument()
    ...
    await user.click(screen.getByRole('tab', { name: 'Zwischenstand' }))
    await screen.findByRole('tab', { name: 'Zwischenstand', selected: true })
    expect(window.location.pathname + window.location.search).toBe('/shows/1/evaluation?view=standings')
    expect(await screen.findByRole('heading', { name: 'Zwischenstand' })).toBeVisible()
    expect(screen.getByText('1 abgestimmt · 1 nicht abgestimmt · 3 unerfasst')).toBeVisible()
